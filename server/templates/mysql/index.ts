...
const { generateSnowflakeSchema } = require('./models/modelsGenerator');
...
const { generateIndexRoute, routesGenerator } = require('./routes/routesGenerator');
...
const createDbModels = async (options: DbModelsInput) => {
    ...
    if (!fs.pathExistsSync(middlewareFile)) {
        ...
        fs.writeFileSync(
            middlewareFile,
            beautify(generateMiddlewareFile(pluralCollectionName, singularCollectionName))
        );
    }
